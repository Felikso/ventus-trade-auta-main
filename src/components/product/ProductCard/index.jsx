...
export const ProductCard = ({ bgImage, product, i, sellIcon, icons, iconsArray }) => {
    ...
    const icon = product.sold.checkboxValueOptions[0].value ? sellIcon?.localFile.childSvg : null

    const img = product.sold.checkboxValueOptions[0].value ? sellIcon?.localFile.childImageSharp : null

    const contentArray = [
        product.price + " pln", 
        product.invoice.checkboxValueOptions[0].value ? "tak" : "nie", 
        product.oil, 
        product.courseValue, 
    ]
